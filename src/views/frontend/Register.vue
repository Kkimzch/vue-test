          ...
          <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
            ...
            <div class="w-full">
              ...
              <form @submit.prevent="onsubmit">
                ...
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none" id="fullname" name="fullname" type="text">
                ...
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none" id="username" name="username" type="text">
                ...
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none" id="mobile" name="mobile" type="text">
                ...
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none" id="email" name="email" type="text">
                ...
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none" id="password" name="password" type="password">
    
                <label class="block mt-3 mb-2 text-sm" for="confirm_password">ยืนยันรหัสผ่าน</label>
                <div v-if="v$.password.comfirm_password.$error" class="text-sm mt-2 text-red-700">{{ v$.password.comfirm_password.$errors[0].$message }}</div>
                <input v-model="password.comfirm_password"
                :class="{'border-red-600': v$.password.comfirm_password.$errors.length}"
                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none" id="confirm_password" name="confirm_password" type="password">
                ...
                <input @click="submitFrom" type="submit" class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg cursor-pointer active:bg-purple-600 hover:bg-purple-700" value="สมัครสมาชิก">
              </form>
...
export default {
  ...
  validations(){
    return{
      fullname: {
        required:helpers.withMessage('กรอกชื่อ - นามสกุล',required),
      },
      username:{
        required:helpers.withMessage('กรอกชื่อผู้ใช้',required),
        validUsername:helpers.withMessage('a-z/A-Z',validUsername),
      },
      mobile:{
        required:helpers.withMessage('กรอกชื่อเบอร์โทรศัพท์',required),
      },
      email:{
        required:helpers.withMessage('กรอกอีเมล',required),
        email:helpers.withMessage('รูปแบบอีเมลไม่ถูกต้อง',email),
      },
      password:{
        password:{
          required:helpers.withMessage('กรอกรหัสผ่าน',required),
          min:helpers.withMessage('รหัสขั้นต่ำความยาว 6 ตัวอักษร', minLength(6)),
        },
        comfirm_password:{
          required:helpers.withMessage('กรอกรหัสผ่าน',required),
          sameAs:helpers.withMessage('รหัสผ่านไม่ตรงกัน',sameAs(this.password.password))
        }
      }
    }
  },
        ...
        http.post('register',
          {
            ...
            "password_confirmation": this.password.comfirm_password,
            "tel":this.mobile,
            "role": 1
         }
        ).then(response => {
